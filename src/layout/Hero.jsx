import { motion } from "framer-motion";
import MotionHeading from "../components/Heading";

export default function Hero() {
	const variants = {
		hidden: { opacity: 0, x: "-150px" },
		headingShow: {
			opacity: 1,
			x: "0px",
			transition: { duration: 1, delay: 0.7, ease: "easeInOut" }
		},
		textShow: {
			opacity: 1,
			x: "0px",
			transition: { duration: 1.25, delay: 1.2, ease: "easeInOut" }
		},
		linkShow: {
			opacity: 1,
			x: "0px",
			transition: { duration: 0.35, delay: 1.4, ease: "easeInOut" }
		},
		imageHidden: { opacity: 0, y: "200px" },
		imageShow: {
			opacity: 1,
			y: "0px",
			transition: { duration: 1.2, delay: 1.5, ease: "easeInOut" }
		}
	};

	return (
		<section
			className="flex flex-col items-center gap-6 px-6 py-28 md:px-10 lg:px-15"
			id="home"
		>
			<div className="flex flex-col items-center gap-8">
				<div className="flex max-w-[600px] flex-col items-center gap-5 text-center lg:max-w-[800px]">
					<MotionHeading
						variants={variants}
						initial="hidden"
						animate="headingShow"
					>
						Transform Your Journey with Daily Stride
					</MotionHeading>
					<motion.p
						variants={variants}
						initial="hidden"
						animate="textShow"
						className="lg:max-w-[600px]"
					>
						At Daily Stride, we provide you with the tools to build lasting
						habits, track your progress, and reflect on your journey—all in one
						intuitive platform. Start today and see the difference with each
						step you take.
					</motion.p>
				</div>
				<motion.a
					variants={variants}
					initial="hidden"
					animate="linkShow"
					className="cursor-pointer rounded-full border-2 border-transparent bg-primaryRed-200 px-3 py-2 text-primaryRed-800 outline-none transition-all duration-300 ease-in-out hover:border-primaryRed-800 hover:bg-primaryRed-100 focus:ring focus:ring-transparent focus:ring-offset-2 focus:ring-offset-primaryRed-800 dark:hover:border-primaryRed-400 dark:focus:ring-offset-primaryRed-400 md:px-4 md:py-3"
					href="#signup"
				>
					Get Started
				</motion.a>
			</div>
			<motion.figure
				variants={variants}
				initial="imageHidden"
				animate="imageShow"
				className="w-96 sm:w-[450px] md:w-[700px] lg:w-[1000px]"
			>
				<img
					src="/assets/hero.webp"
					alt="Mockup of Daily Stride dashboard showcasing the main dashboard for desktops and phone devices."
					loading="lazy"
				/>
			</motion.figure>
		</section>
	);
}
